...
abstract class IntegrationCommandBase extends CommandBase
{
     ...
     * @param Project $project
     */
    protected function ensureHooks(Integration $integration, array $creationValues, Project $project)
    {
        ...
        try {
            $hooks = \GuzzleHttp\json_decode($client->get($hooksApiUrl, $requestOptions)->getBody()->__toString(), true);
            ...
            elseif ($this->hookNeedsUpdate($integration, $hook, $payload)) {
                ...
                $client->send(
                    new Request($method, $hookApiUrl, ['body' => \json_encode($payload)] + $requestOptions)
                );
