            ...
            switch ($e->getEnvironment()->status) {
                ...
                case 'paused':
                    ...
                    if ($this->config()->isCommandEnabled('environment:resume')) {
                        $this->stdErr->writeln('');
                        $this->stdErr->writeln(sprintf('Resume the environment by running: <info>%s resume -e %s</info>', $this->config()->get('application.executable'), OsUtil::escapeShellArg($environment->id)));
                    }
                    return 1;
            }
            throw $e;
