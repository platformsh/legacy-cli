...
class DiskUsageCommand extends CommandBase
{
    ...
    protected function configure()
    {
        ...
        $this->setName('metrics:disk-usage')
            ...
            ->addOption('range', 'r', InputOption::VALUE_REQUIRED,
                ...
                . "\n" . \sprintf(
                    'Minimum <comment>%s</comment>, maximum <comment>%s</comment>, default <comment>%s</comment>.',
                    $duration->humanize(self::MIN_RANGE),
                    $duration->humanize(self::MAX_RANGE),
    ...
    {
        // TODO remove this warning when the command is finished
        $this->api()->getClient(); // Trigger login before the warning so the output has more context.
        $this->stdErr->writeln('<options=bold;fg=yellow>Warning</>');
        $this->stdErr->writeln('This command is experimental. The options and API may change in future.');
        $this->stdErr->writeln('');
        ...
        if ($rangeStr = $input->getOption('range')) {
            ...
            if (empty($rangeSeconds)) {
                ...
                return 1;
            } elseif ($rangeSeconds > self::MAX_RANGE) {
                $this->stdErr->writeln(\sprintf('The --range <error>%s</error> is too long: it must be %d seconds (%s) or less.', $rangeStr, self::MAX_RANGE, (new Duration())->humanize(self::MAX_RANGE)));
                return 1;
            }
        ...
        if ($serviceName) {
            ...
            $serviceName = $questionHelper->choose($choices, 'Enter a number to choose a service');
            if ($serviceName === null) {
        ...
        if (!$table->formatIsMachineReadable()) {
            ...
            $this->stdErr->writeln(\sprintf(
                'Disk usage every <info>%s</info> from <info>%s</info> to <info>%s</info>:',
        ...
        foreach ($valuesByTimestamp as $time => $values) {
            ...
            foreach (['', 'i'] as $prefix) {
                if (isset($values[$prefix . 'used']['value']['max'])) {
                    ...
                    $row[$prefix . 'used'] = $bytes || $prefix === 'inodes' ? $value : FormatterHelper::formatMemory($value);
                }
                if (isset($values[$prefix . 'limit']['value'])) {
                    if (isset($values[$prefix . 'limit']['value']['min'])) {
                        $value = $values[$prefix . 'limit']['value']['min'];
                        $row[$prefix . 'limit'] = $bytes || $prefix === 'inodes' ? $value : FormatterHelper::formatMemory($value);
                    }
