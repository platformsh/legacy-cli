...
abstract class AbstractMetricsCommandBase extends CommandBase
{
     ...
     */
    protected function processQuery(InputInterface $input, array $metricTypes): array
    {
        // Common
        ...
        $env = $environment->getCurrentDeployment()->getData();
        $region = explode('.', parse_url($env['_links']['self']['href'], PHP_URL_HOST), 2)[0];

        $link = 'https://admin-pipeline.upsun-' . $region . '.observability-pipeline.blackfire.io/platformsh-api/' . $env['project_info']['name'] . '/' . $env['environment_info']['machine_name'] . '/resources/overview';
        ...
        foreach ($values['data'] as $point) {
            ...
            foreach ($byService as $service => $byDimension) {
                ...
                foreach ($fieldMapping as $field => $fieldDefinition) {
                    ...
                    //                    if (isset($byDimension[$fieldName])) {
                    //                        /** @var Sketch $value */
                    //                        $value = $byDimension[$fieldName];
                    //                        if ($fieldName === 'mem_percent' && isset($deployment->services[$service])) {
                    //                            if ($value->average() > 90) {
                    //                                $this->foundHighMemoryServices = true;
                    //                            }
                    //                            $row[$columnName] = $field->format($byDimension[$fieldName][$field->aggKind], false);
                    //                        } elseif ($fieldName === 'mem_limit' && $service === 'router' && $value->average() == 0) {
                    //                            $row[$columnName] = '';
                    //                        } elseif ($fieldName === 'mem_percent' && $service === 'router' && $value->isInfinite()) {
                    //                            $row[$columnName] = '';
                    //                        } else {
                    //                            $row[$columnName] = $field->format2($byDimension[$fieldName][$field->aggKind]);
                    //                        }
                    //                    }
                    //                }
                    //                    foreach ($metricKinds as $metricKind) {
